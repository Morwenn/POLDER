...
bool fgetl(char** line, FILE* f)
{
    ...
    if (size == 1) return false; // If the only character is EOF, return NULL
...
////////////////////////////////////////////////////////////
char* read_word(const char* str, unsigned int searched_word_number)
{
...
char* read_word_first(const char* str)
{
    ...
    read_word_str = new char[read_word_length];
...
char* read_word_next()
{
    ...
    for (; read_word_index <= read_word_length ; ++read_word_index)
    {
        if (!isspace(read_word_str[read_word_index])
            && read_word_str[read_word_index] != '\0'
...
char* substr(const char* str, size_t index_begin, size_t index_end)
{
    ...
    {
        register int temp = index_end - index_begin + 1;
