...
#    define CATCH_CONFIG_CPP11_CONSTEXPR
#  endif
...
// constexpr support
#if defined(CATCH_CONFIG_CPP11_CONSTEXPR) && !defined(CATCH_CONSTEXPR)
#  define CATCH_CONSTEXPR constexpr
#else
#  define CATCH_CONSTEXPR
#endif
...
// compile time abs and max functions
#ifdef CATCH_CONFIG_CPP11_CONSTEXPR
CATCH_CONSTEXPR double abs(double val) {
    return (val >= 0.0) ? val : -val;
}
CATCH_CONSTEXPR double (max)(double lhs, double rhs) {
    ...
    class Approx {
    public:
        explicit CATCH_CONSTEXPR Approx ( double value )
        :   m_epsilon( std::numeric_limits<float>::epsilon()*100 ),
            m_scale( 1.0 ),
            m_value( value )
        {}

        CATCH_CONSTEXPR Approx( Approx const& other )
        :   m_epsilon( other.m_epsilon ),
            m_scale( other.m_scale ),
            m_value( other.m_value )
        {}

        static CATCH_CONSTEXPR Approx custom() {
            return Approx( 0 );
        }
        ...
        friend CATCH_CONSTEXPR bool operator == ( double lhs, Approx const& rhs ) {
            // Thanks to Richard Harris for his help refining this formula
#ifdef CATCH_CONFIG_CPP11_CONSTEXPR
            return abs( lhs - rhs.m_value ) < rhs.m_epsilon * (rhs.m_scale + (max)( abs(lhs), abs(rhs.m_value) ) );
#else
            return fabs( lhs - rhs.m_value ) < rhs.m_epsilon * (rhs.m_scale + (std::max)( fabs(lhs), fabs(rhs.m_value) ) );
#endif
        }

        friend CATCH_CONSTEXPR bool operator == ( Approx const& lhs, double rhs ) {
